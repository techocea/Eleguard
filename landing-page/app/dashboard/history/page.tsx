...
type FilterStatusType = "ALL" | SensorStatus;

export default function HistoryPage() {
  const [data, setData] = useState<HistoricalLogItem[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setData(res.data.items as HistoricalLogItem[]);
  ...
  const filtered = data.filter((item) => {
    const matchSearch =
      item.sensorId.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const formatTime = (ts: string): FormattedTimeObject => {
    const d = new Date(ts);
    return {
  ...
  const stats: HistoricalStats = {
    ...
    thisWeek: data.filter((d) => {
      ...
      const now = new Date();
      return now.getTime() - d2.getTime() < 7 * 24 * 60 * 60 * 1000;
    }).length,
  };
    ...
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "#5F6B63" }}>
          Complete log of elephant detection events
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          >
            <div className="text-xs mb-1" style={{ color: "#5F6B63" }}>
              {label}
            </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          {(
            ["ALL", "CRITICAL", "HIGH", "MEDIUM", "LOW"] as FilterStatusType[]
          ).map((s) => {
            // Map to THREAT_WEIGHT safely, defaulting to 0 for 'ALL' or unmatched entries
            const weight = s !== "ALL" ? THREAT_WEIGHT[s] : 0;
            const colors =
              s !== "ALL" ? THREAT_COLORS[threatLevelToColor(weight)] : null;
            ...
            return (
              <button
                ...
                style={
                  filterStatus === s
                    ? {
                        background: colors?.bg || "rgba(129,199,132,0.2)",
                        color: colors?.dot || "#81C784",
                        border: `1px solid ${colors?.border || "#81C784"}`,
                      }
                    : {
                        background: "rgba(27,35,30,0.6)",
                        color: "#5F6B63",
                        border: "1px solid rgba(129,199,132,0.1)",
                      }
                }
      ...
      >
        <div
          ...
          style={{ borderColor: "rgba(129,199,132,0.08)" }}
        >
          <div
            className="grid grid-cols-5 text-xs font-bold uppercase tracking-widest"
            style={{ color: "#3D5C41" }}
          >
            <div>Sensor</div>
            <div>Status</div>
        ...
        {loading ? (
          ...
          paginated.map((item, i) => {
            ...
            const weight = THREAT_WEIGHT[item.status] || 0;
            const colors = THREAT_COLORS[threatLevelToColor(weight)];
            return (
              <div
                key={i}
                ...
                style={{
                  borderBottom:
                    i < paginated.length - 1
                      ? "1px solid rgba(129,199,132,0.05)"
                      : "none",
                }}
              >
                ...
                <div className="font-mono text-sm" style={{ color: "#81C784" }}>
                  {time}
                </div>

                <div className="text-sm" style={{ color: "#5F6B63" }}>
                  {item.duration}
                </div>
              </div>
            );
