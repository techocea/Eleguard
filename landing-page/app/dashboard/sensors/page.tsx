...
function SensorPopup({
  ...
  const [form, setForm] = useState<SensorFormData>(existingData || EMPTY_FORM);
  const [saving, setSaving] = useState<boolean>(false);

  const set =
    (k: keyof SensorFormData) =>
    (
      e: React.ChangeEvent<
      ...
      >
        <div
          style={{
            ...
            overflow: "hidden",
            background:
              "linear-gradient(145deg, rgba(20,32,22,0.99), rgba(12,17,14,0.99))",
            border: "1px solid rgba(129,199,132,0.2)",
            boxShadow:
              "0 28px 60px rgba(0,0,0,0.8), inset 0 1px 0 rgba(129,199,132,0.1)",
        ...
        >
          ...
          <div
            style={{
              ...
              background:
                "linear-gradient(135deg,rgba(27,94,32,0.22),transparent)",
            }}
          >
            <div
              style={{
                ...
                background:
                  "linear-gradient(90deg,transparent,#81C784 40%,#A5D6A7 60%,transparent)",
              }}
            />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: "#5F6B63", marginTop: 1 }}>
                  {/* {SENSOR_META[sensorId]?.desc} ·{" "} */}
                  {SENSOR_META[sensorId]?.side?.toUpperCase()} boundary
                </div>
              </div>
              <button
                onClick={onCancel}
            ...
            >
              🔬 This data trains the ML model to predict elephant movement
              patterns near this sensor location.
            </div>
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                style={inputSt}
                onFocus={(e) =>
                  (e.target.style.borderColor = "rgba(129,199,132,0.6)")
                }
                onBlur={(e) =>
                  (e.target.style.borderColor = "rgba(129,199,132,0.2)")
                }
              />
            </div>
            ...
            >
              <div>
                ...
                >
                  <Droplets size={11} style={{ color: "#3D5C41" }} /> WATER
                  CANAL (m)
                </label>
                <input
                  type="number"
            ...
            <div>
              ...
              >
                <Eye size={11} style={{ color: "#3D5C41" }} /> ANIMAL ACTIVITY
                NOTICED?
              </label>
              <select
                value={form.animalActivity}
                onChange={set("animalActivity")}
                style={inputSt}
                onFocus={(e) =>
                  (e.target.style.borderColor = "rgba(129,199,132,0.6)")
                }
            ...
            <div>
              ...
              >
                <FileText size={11} style={{ color: "#3D5C41" }} /> ADDITIONAL
                NOTES
              </label>
              <textarea
                value={form.notes}
...
// ── Sensor Card ───────────────────────────────────────────────────────────────
function SensorCard({ id, active, hasData, onToggle }: SensorCardProps) {
  const meta: SensorMetaItem | undefined = SENSOR_META[id];

  // Defensive check if an unexpected ID slips into your data mapping loop
  if (!meta) return null;
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          fontSize: 10,
          color: active ? "rgba(255,205,210,0.55)" : "#3D5C41",
          marginBottom: 3,
        }}
      >
        {/* {meta.desc} */}
...
export default function SensorControlsPage() {
  const [sensorData, setSensorData] = useState<Record<string, SensorFormData>>(
    {},
  );
  const [activeSet, setActiveSet] = useState<Set<string>>(new Set());
  const [popup, setPopup] = useState<string | null>(null);

  // 2. Strongly Typed Callback Pipeline Handlers
  const handleSave = useCallback(
    (id: string, formData: SensorFormData): void => {
  ...
  // 3. Derived Variables Types
  const activeSensors: string[] = [...activeSet];
  const totalSensors: number = Object.keys(SENSOR_META).length;
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "#5F6B63" }}>
          Manually activate sensors. When turning ON, you'll be asked to log
          cultivation data for ML training.
        </p>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: "#3D5C41" }}>
          {activeSensors.length} / {totalSensors} active ·{" "}
          {Object.keys(sensorData).length} data records collected
        </div>
      ...
      >
        <ChevronRight
          size={14}
          style={{ color: "#4CAF50", flexShrink: 0, marginTop: 2 }}
        />
        <div style={{ fontSize: 11, color: "#5F6B63", lineHeight: 1.6 }}>
          <strong style={{ color: "#81C784" }}>How it works:</strong> Click any
          sensor card to toggle ON/OFF. When activating, a form appears to
          collect cultivation & environment data for that sensor location. This
          data trains the AI prediction model to improve accuracy.
          <br />
          <span style={{ color: "#3D5C41" }}>
            Green dot = data already collected · Red dot = sensor active
          </span>
        </div>
      </div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#B0BEC5" }}>
              Sensors recording — visit AI Prediction page to see predicted
              threat zones
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {Object.entries(sensorData).map(([id, data]) => (
              ...
              >
                ...
                <span style={{ color: "#3D5C41" }}>
                  {data.cropType || "No crop specified"}
                </span>
                <span style={{ color: "#2A3C2C", fontSize: 9 }}>
                  {data.timestamp
