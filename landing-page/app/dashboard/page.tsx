...
import { HeatmapGrid, SensorMetaMap } from "@/types";
...
//   col-3/4    → ~0.30 (YELLOW)
//   far left   → ~0.05 (GREEN)
const buildGrid = (activeSensors: string[]): HeatmapGrid => {
  ...
  for (const sid of activeSensors) {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        const sigma = 4.5;
        const lateralHeat = Math.exp(
          -(lateral * lateral) / (2 * sigma * sigma),
        );
  ...
  return (
    <div
      ...
      style={{
        ...
        background: active ? "rgba(160, 0, 0, 0.88)" : "rgba(22, 52, 28, 0.85)",
        border: active
          ? "2px solid #D32F2F"
          : "2px solid rgba(56, 110, 60, 0.45)",
        color: active ? "#FFCDD2" : "#4CAF50",
        boxShadow: active ? "0 0 18px rgba(239,83,80,0.7)" : "none",
        animation: active
...
export default function HeatMap() {
  ...
  const activeSensors = useMemo<string[]>(
    () => [...active] as string[],
    [active],
  );
    ...
    <div style={{ fontFamily: "'Nunito', sans-serif" }}>
      ...
      >
        ...
        >
          ...
          <button
            onClick={() =>
              setMode((m) => (m === "manual" ? "backend" : "manual"))
            }
            style={{
              padding: "7px 14px",
              borderRadius: 10,
        ...
        >
          <AlertTriangle
            ...
            style={{
              color: "#EF5350",
              flexShrink: 0,
              animation: "dotBlink 0.5s linear infinite",
            }}
          />
          <div
            style={{
      ...
      {mode === "manual" && (
        <div
          style={{
            ...
            padding: "6px 10px",
            borderRadius: 8,
            background: "rgba(27,35,30,0.5)",
            border: "1px solid rgba(129,199,132,0.08)",
          }}
        >
          💡 Sensor button eka click karanna — on/off wenawa. Cultivation area
          eke cells real-time color wenawa.
        </div>
      )}
        ...
        >
          ...
          >
            {[
              ...
              {
                color: "#FF8F00",
                bg: "rgba(154,56,0,0.9)",
                label: "MED",
                sub: "Approaching",
              },
              {
                color: "#F9A825",
                bg: "rgba(122,88,0,0.9)",
                label: "LOW",
                sub: "Distant",
              },
              {
